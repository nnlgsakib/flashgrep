...
The system MUST define a machine-readable tool routing policy that prioritizes Flashgrep-native tools for search, read, write, and symbol workflows before any native or generic alternatives, and this policy MUST be delivered during initialization from a deterministic embedded source. For agent search workflows, this policy MUST prioritize programmatic retrieval with explicit deterministic fallback gates for non-Flashgrep-native execution.
...
#### Scenario: Search policy declares programmatic retrieval order
