...
### `flashgrep fs` (CLI fallback for filesystem lifecycle)
Use when the task requires filesystem lifecycle operations not covered by MCP read/write primitives.

Recommended operations:
- `flashgrep fs create`
- `flashgrep fs list --sort-by path --sort-order asc --offset ... --limit ...`
- `flashgrep fs stat --output json`
- `flashgrep fs copy` / `flashgrep fs move` with explicit `--overwrite` when needed
- `flashgrep fs remove --dry-run` before destructive operations
